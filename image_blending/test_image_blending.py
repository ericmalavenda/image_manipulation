...
def run_blend(black_image, white_image, mask):
  ...
  gauss_pyr_mask = homework4.gaussPyramid(mask, depth)
  gauss_pyr_black = homework4.gaussPyramid(black_image, depth)
  gauss_pyr_white = homework4.gaussPyramid(white_image, depth)


  lapl_pyr_black  = homework4.laplPyramid(gauss_pyr_black)
  lapl_pyr_white = homework4.laplPyramid(gauss_pyr_white)

  outpyr = homework4.blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask)
  outimg = homework4.collapse(outpyr)
...
def test_reduce_expand():
  ...
  for red_pyr in reduce1, reduce2:
    for imgin, true_out in zip(red_pyr[0:-1], red_pyr[1:]):
      ...
      usr_out = homework4.reduce(imgin)
  ...
  # each subsequent layer is an expansion of the previous one  
  ...
  for imgin, true_out in zip(expandin, expandout):
    ...
    usr_out = homework4.expand(imgin)
...
def test_gaussian_laplacian():
  ...
  for pyr in gauss_pyr1, gauss_pyr2:
    ...
    usr_out = homework4.gaussPyramid(pyr[0], 3)
  ...
  for gauss_pyr, lapl_pyr in zip((gauss_pyr1, gauss_pyr2), (lapl_pyr1, lapl_pyr2)):
    ...
    usr_out = homework4.laplPyramid(gauss_pyr)
...
def test_blend_collapse():
  ...
  for left_pyr, right_pyr, mask_pyr, out_pyr in ((lapl_pyr11, lapl_pyr12, mask_pyr1, out_pyr1), 
    ...
    usr_out = homework4.blend(left_pyr, right_pyr, mask_pyr)
  ...
  for pyr, img in ((out_pyr1, outimg1),(out_pyr2, outimg2)):
    ...
    usr_out = homework4.collapse(pyr)
