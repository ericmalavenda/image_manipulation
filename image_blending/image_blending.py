...
fname1 = 'Mars.jpg' # file name
cv1img = cv2.imread(fname1)
## convert to grayscale
gray_scale1 = cv2.cvtColor( cv1img, cv2.COLOR_RGB2GRAY )
fname2 = 'masq.jpg' # file name
cv2img = cv2.imread(fname2)
## convert to grayscale
gray_scale2 = cv2.cvtColor( cv2img, cv2.COLOR_RGB2GRAY )
cv2_alpha7_add_result = cv2.addWeighted(gray_scale1, 0.3, gray_scale2, 0.7,0)
cv2.imshow('new mask', cv2_alpha7_add_result)
cv2.waitKey(0)
cv2.imwrite('mask.jpg',cv2_alpha7_add_result)
 """ 
def generatingKernel(parameter): ### passed

  kernel = np.array([0.25 - parameter / 2.0, 0.25, parameter,
                         0.25, 0.25 - parameter /2.0])
  return np.outer(kernel, kernel)

def reduce(image): ### passed
  ...
  """
  # def reduce(): convolve ---> subsample
  ### Step 1: "Convolve the input image with a generating kernel of parameter of 0.4..."
  ...
  ### "subsampling (essentially you want to index every other row and every other column)."
  return reduced_img_convolved
  
def expand(image): ### passed
  ...
  """
  # def expand(): upsample ---> convolve
  ### Step 1: "Expand the image to double the size..." 
  (x, y) = image.shape
  expander = np.zeros([x * 2, y * 2])
  ### Step 2: "...and then convolve it with a generating
  ### kernel with a parameter of 0.4. ***Note: You should upsample the image, and 
  ### then convolve it with a generating kernel of a = 0.4.***"
  expander[::2, ::2] = image
  return (scipy.signal.convolve2d(expander, generatingKernel(0.4), 'same') * 4) 
  ### Step 3: "Finally, multiply your output image by a factor of 4 
  ### in order to scale it back up. If you do not do this 
  ### (and I recommend you try it out without that)
  ### you will see that your images darken as you apply the 
  ### convolution. Please explain why this happens in your submission PDF."

def gaussPyramid(image, levels): ### passed
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  for layer in range(len(laplPyrBlack)):
    _layer_ = laplPyrBlack[layer] * (1 - gaussPyrMask[layer]) + laplPyrWhite[layer] * gaussPyrMask[layer]
